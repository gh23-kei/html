...
def draw_glow_text(glow_layer, screen_layer, font, word, color, x, y, alpha, size_mult=1.0):
    ...
    # Sử dụng SysFont thống nhất để tránh lỗi khởi tạo None Font
    if size_mult != 1.0:
...
def main():
    ...
    # SỬA LỖI NHẠC: Chỉ play khi file thực sự tồn tại, tránh bị crash
    if os.path.exists("love_you.mp3"):
        pygame.mixer.music.load("love_you.mp3")
        pygame.mixer.music.play(-1) # -1 để lặp lại bài hát liên tục
